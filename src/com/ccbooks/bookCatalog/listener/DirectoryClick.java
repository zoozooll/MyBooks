package com.ccbooks.bookCatalog.listener;


import java.util.ArrayList;

import com.ccbooks.adapter.DirectoryAdapter;
import com.ccbooks.view.BookCatalogView;
import com.ccbooks.view.BookContentView;
import com.ccbooks.view.BookListView;
import com.ccbooks.view.TextReader;
import com.chinachip.books.plugin.Plugin;
import com.chinachip.tree.Node;



import android.app.Activity;
import android.content.Context;
import android.content.Intent;
import android.content.SharedPreferences;
import android.os.Bundle;
import android.view.View;

import android.widget.AdapterView;
import android.widget.Toast;

import android.widget.AdapterView.OnItemClickListener;

public class DirectoryClick implements OnItemClickListener{
	
	private BookCatalogView bcv;
	private DirectoryAdapter directoryAdapter; //亮度调节悬浮框；
	private Context context;

	public DirectoryClick(Context context,DirectoryAdapter directoryAdapter) {
		super();
		this.bcv = (BookCatalogView)context;
		this.directoryAdapter = directoryAdapter;
		this.context = context;
		
	}

	
	
	// 点击背景按钮�?
	
	

		
	        
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				long arg3) {
			directoryAdapter.selected = (int) arg3;
			if(bcv.nodeList.size() == 1)
			{	
				Node tempNode = bcv.nodeList.get(directoryAdapter.selected);
				ArrayList<Node> nodeList = bcv.myTree.getChildNodeList(tempNode);
				if(nodeList.size() != 0){
					bcv.nodeList = nodeList;
					directoryAdapter.setNodeList(nodeList);
				}else{
//					Plugin plugin = BookContentView.bc.pm.getDefaultPlugin();
//					BookContentView.bc.pm.close(plugin, BookContentView.bc.pm.filehandle);
					Intent intent = new Intent();
					Bundle bundle = new Bundle();
					int file = bcv.nodeList.get(0).file;
					
					Plugin plugin = BookContentView.bc.pm.getDefaultPlugin();
					int filehandle = BookContentView.bc.pm.filehandle;
					BookContentView.bc.pm.close(plugin, filehandle);
					filehandle = BookContentView.bc.pm.open(plugin, file);
					long fileLength = BookContentView.bc.pm.length(plugin, filehandle);
					if(fileLength == 0){
						
						Toast.makeText(context, "此章节内容为空!", Toast.LENGTH_SHORT).show();
						return;
					}
					BookContentView.bc.setFilehandle(filehandle);
					BookContentView.bc.resetFile();
					bundle.putInt("file", file);
					bundle.putInt("bookId", bcv.id);
					
					
					intent.putExtras(bundle);
					bcv.setResult(0,intent);
					bcv.finish();
				}
				
			}else{
				Node tempNode = bcv.nodeList.get(directoryAdapter.selected);
				
				
				ArrayList<Node> nodeList = bcv.myTree.getChildNodeList(tempNode);
				if(nodeList.size() != 0){
					bcv.nodeList = nodeList;
					directoryAdapter.setNodeList(nodeList);
				}else{
//					Plugin plugin = BookContentView.bc.pm.getDefaultPlugin();
//					BookContentView.bc.pm.close(plugin, BookContentView.bc.pm.filehandle);
					Intent intent = new Intent();
					Bundle bundle = new Bundle();
					int file = bcv.nodeList.get((int)arg3).file;
					Plugin plugin = BookContentView.bc.pm.getDefaultPlugin();
					int filehandle = BookContentView.bc.pm.filehandle;
					BookContentView.bc.pm.close(plugin, filehandle);
					filehandle = BookContentView.bc.pm.open(plugin, file);
					long fileLength = BookContentView.bc.pm.length(plugin, filehandle);
					if(fileLength == 0){
						
						Toast.makeText(context, "此章节内容为空!", Toast.LENGTH_SHORT).show();
						return;
					}
					BookContentView.bc.setFilehandle(filehandle);
					BookContentView.bc.resetFile();
					bundle.putInt("file", file);
					bundle.putInt("bookId", bcv.id);
					
					intent.putExtras(bundle);
					bcv.setResult(0,intent);
					bcv.finish();
				}
			}
			if(bcv.myTree.isRoot(bcv.nodeList.get(0)))
			{
				bcv.btBack.setVisibility(View.INVISIBLE);
			}else{
				bcv.btBack.setVisibility(View.VISIBLE);
			}
			
		
			
			directoryAdapter.notifyDataSetChanged();
			
			//list.setSelectionAfterHeaderView();
		}


		
	

	
}
