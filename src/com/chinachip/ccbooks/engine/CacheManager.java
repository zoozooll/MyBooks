...
public class CacheManager
{
  ...
  public int getNextDataBlock() throws IOException
  {
    ...
    {
      loadDataBlock(this.pm, getNextDataBlockStart(this.cache[this.currentCacheID].getBlockId()), tempID);
      return tempID;
    }
    ...
    throws IOException
  {
    ...
    if (tempID != this.currentCacheID) {
      loadDataBlock(this.pm, getPrevDataBlockStart(this.cache[this.currentCacheID].getBlockId()), tempID);
      return tempID;
    }
  ...
  public int getCacheFromOffset(long offset) throws IOException
  {
    ...
    if (i >= this.cache.length)
    {
      if (this.cache[this.currentCacheID].getBlockId() != -1) {
        this.currentCacheID = ChooseOptimize(this.cache[this.currentCacheID].getStart());
      }
  ...
  private boolean isHas(int i, long offset) {
    ...
    {
      nextStart = ((Long)this.blockIndex.get(this.blockIndex.size() - 1)).longValue();
    }
    else {
      nextStart = ((Long)this.blockIndex.get(this.cache[i].getBlockId())).longValue();
    }
    ...
    throws IOException
  {
    ...
    if (offset >= filelength) {
      start = ((Long)this.blockIndex.get(this.blockIndex.size() - 2)).longValue();
    ...
    for (int k = 1; k < this.blockIndex.size(); k++) {
      if (((Long)this.blockIndex.get(k)).longValue() <= offset)
        continue;
      start = ((Long)this.blockIndex.get(k - 1)).longValue();
      length = ((Long)this.blockIndex.get(k)).longValue() - ((Long)this.blockIndex.get(k - 1)).longValue();
      mbb = pm.map(plugin, this.filehandle, start, length);
      this.data = new byte[(int)length];
      mbb.get(this.data);
      String text = new String(this.data, this.enc);
      ...
      this.cache[cacheId].setData(text);
      this.cache[cacheId].setStart(((Long)this.blockIndex.get(k - 1)).longValue());
      getParagraphIndex(this.cache[cacheId]);
      return;
    }
  }

  private int loadNextDataBlock(PluginMgr pm, int blockId)
    throws IOException
  {
    int i = ChooseOptimize(((Long)this.blockIndex.get(blockId - 1)).longValue());

    Plugin plugin = pm.getDefaultPlugin();
    long filelength = pm.length(plugin, this.filehandle);
    long start = ((Long)this.blockIndex.get(blockId - 1)).longValue();
    long length = 0L;
    if (blockId < this.blockIndex.size() - 1)
      length = ((Long)this.blockIndex.get(blockId)).longValue() - start;
    else
      length = filelength - start;
    mbb = pm.map(plugin, this.filehandle, start, length);
    this.data = new byte[(int)length];
  ...
  private int loadPrevDataBlock(PluginMgr pm, int blockId)
    throws IOException
  {
  ...
  private long getNextDataBlockStart(int blockId)
    throws IOException
  {
    if (blockId >= ((Long)this.blockIndex.get(this.blockIndex.size() - 2)).longValue()) {
      return ((Long)this.blockIndex.get(this.blockIndex.size() - 2)).longValue();
    }
    return ((Long)this.blockIndex.get(blockId)).longValue();
  }

  private long getPrevDataBlockStart(int blockId) throws IOException {
    if (blockId <= 1) {
      return ((Long)this.blockIndex.get(0)).longValue();
    }
    return ((Long)this.blockIndex.get(blockId - 2)).longValue();
  }
  public void setCurrentCacheID(int currentCacheID) {
    this.currentCacheID = currentCacheID; } 
  private void getParagraphIndex(Cache cache) { ArrayList paragraphIndex = new ArrayList();
    String data = cache.getData();
    int strLen = data.length();
    int indexAt = 0;
    paragraphIndex.add(0, Integer.valueOf(0));
    int temp;
    do { temp = data.indexOf("\n") + 1;
  ...
  public long getFileLength()
  {
    ...
    if (fileLength > 0L) return fileLength;
    return 0L;
  }
